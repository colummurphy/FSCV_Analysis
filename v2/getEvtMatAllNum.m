...
%operations
%Limit 3rd dimension (number of events stored per trial) to 5
...
limev=5;%Limit of events per trial
...
evmatrix=nan(length(trids),length(plotevents),limev);
evmatFSCV=nan(length(trids),length(plotevents),limev);  %From evFSCV, check, fscv TS
evmatFSCVids=nan(length(trids),length(plotevents),limev);  %From evFSCV, but sample IDs rather than fscv TS
evmatFSCVnlx=nan(length(trids),length(plotevents),limev);  %From evFSCV, check -- does not match nlx events (e.g. NlxEventTS, most likely during syncsigs did not explicitly write these in for each ev but relied on interp)
for it=1:length(trids)
    ...
    for ee=1:length(plotevents)
        ...
        if length(targTS)<limev
            nans=nan(limev,1);
            nans(1:length(targTS))=targTS;
            targTS=nans;
        end
        evmatrix(it,ee,1:limev)=targTS(1:limev);  %store array of TSs within defined window, i.e. last out      
        ...
        if length(targIDf)<limev
            nans=nan(limev,1);
            nans(1:length(targIDf))=targIDf;
            targIDf=nans;
        end
       % evmatFSCV(it,ee)={targTSf};
        evmatFSCVids(it,ee,1:limev)=targIDf(1:limev);
