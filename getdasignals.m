...
global processed parameters plotParam hgui
...
allchdata={};
badids=[];
%downsample timestamp lfp id delta ts(N)
downdiv=round(plotParam.ratelfp/parameters.samplerate);   
fscvtswidth=1/round(parameters.samplerate);
lfprawtswidth=0;
if ~isempty(plotParam.ratelfp)    
lfprawtswidth=1/round(plotParam.ratelfp);
end
%processed.glitchids={};
%processed.detected={};
...
for ii=1:length(selch)
    ...
    switch varargin{argnum}  
        ...
        if isfield(processed.Iread(selch(ii)),'events')
            if ~isempty(processed.Iread(selch(ii)).events) && ~(processed.Iread(selch(ii)).events==0)
            ...
            for ilfp=1:length(processed.LFPread.LFPchNames)
               ...
               elseif ismember(ilfp,plotParam.lickid)
                ...
                if ilfp==1
                    ...
                    processed.nlxtsdown=processed.LFPread.LFPts(1:downdiv:end);
            ...
            for iid=1:size(tstrace,1)
                ...
                for ilfp=1:size(processed.resampled,1)
                    if ilfp==1
                        ...
                        if any(targids>length(tsdown))
                            warning(['target ids for lfp trace detected #' ...
                                num2str(iid)...
                                ' exceed stored LFPts; filling tsdown beyond end']);
                            numout=length(find(targids>length(tsdown)));                        
                            %if exceeds ts of ts down, tsdown may be not correct near
                            %longer extensions away from peak
                            %fille tsdown more based on sample rate
                            %nan signal at these extended pts
                            outids=length(tsdown)+1:length(tsdown)+numout;
                            tsdown(outids)=tsdown(end)+...
                                fscvtswidth:fscvtswidth:tsdown(end)+fscvtswidth*numout;
                            for iilfp=1:size(processed.resampled,1)
                            processed.betadown(iilfp,outids)=nan;
                            end
                        end
                        %targids=find(tsdown>=tstrace(iid,1) & tsdown<=tstrace(iid,end))                         
                         tstarg=tsdown(targids); 
                         %mean difference between ts's at center point
                         midpointidx=median(1:size(tstrace,2));
                         tserror=mean(abs(tstrace(iid,midpointidx)-tstarg(midpointidx)));                       
                         if tserror>.1
                             warning(['# ' num2str(iid) ...
                         ...
                         if any(rawtargids>length(rawts))
                            warning(['filling tsrawtarg beyond end']);
                            numout=length(find(rawtargids>length(rawts)));                        
                            %extend tsraw as done with tsdown at lfp sample
                            %rate, nan signal lfp at these extended pts
                            outids=length(rawts)+1:length(rawts)+numout;
                            rawts(outids)=rawts(end)+...
                                lfprawtswidth:lfprawtswidth:rawts(end)+lfprawtswidth*numout;
                            for iilfp=1:size(processed.resampled,1)
                            processed.resampled(iilfp,outids)=nan;
                            end
                         end                     
