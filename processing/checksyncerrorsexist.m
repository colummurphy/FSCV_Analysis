function checksyncerrorsexist(sessnum,align,varargin)
...
if ~strcmp(alignname,'fixedintervals')
    ...
    if length(nlxTargetTSs)==length(nlxTargetTSs2)
        ...
        writelog=[alignname ': discrepancy = ' num2str(discrepancy)];
        %record in log file
        log=[log sprintf('\n') writelog];
    else
    ...
    if str2num(verm(regexp(verm,'\d')))>2016
       [nlxtssfill,tf]=fillmissing(nlxtss,'linear');   %linear interpolate those points
        ...
        %legend('before re-interpolation','after filling');
         writelog=['interpolated fscv tss, saving -' alignname '-nlxTargetTSsinter-....'];
            %record in log file
            save([savefolder alignname '-nlxTargetTSsinter'],'nlxTargetTSs','nlxTargetTSs2');
            log=[log sprintf('\n') writelog];
    else
