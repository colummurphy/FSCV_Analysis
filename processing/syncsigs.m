function syncsigs(subject,sessnum,align,varargin)
%updated 10/2020 so that fixed intervals also checks for alignment of IDs
%with timestamps in both systems incase duplicates or dropped codes, not
%just for individual trial outputs..
...
switch subjectname
    case 'patra'
        run([configdir ['chronic' num2str(sessnum) 'chconfig.m']]);        %get 'paths'
        run([configdir 'patra_map_bipolar.m']);      %get csc_map
    case 'cleo'
        run([configdir ['cleo_chronic' num2str(sessnum) '.m']]);        %get 'paths'
        run([configdir ['cleo_map_bipolar' num2str(sessnum) '.m']]);      %get csc_map & event_codes
end
%selectcsc=cscchs;
alignname='';
...
switch align
    ...
    otherwise
 alignname='bigreward';       %do all
        aligns={'bigreward','smallreward','targetbreak','fixbreak'};        %default
end
...
savepath=fullfile(pathfscv,'matlab',alignname,filesep);
savename=['fscv_multi_' num2str(100) '.mat'];
oldchs=[];
if ~isdir(savepath)
    status = mkdir(savepath);
else
    if addlfps && exist([savepath,savename])>0
        %check existing saved channels
        dataload=load([savepath,savename]);
        if isfield(dataload,'samplesLFP')
            existingchs=dataload.nlxFileNames;
            existingdig=regexp(existingchs,'\d+','match');
            oldchs=str2double([existingdig{:}]);
        end
    end
end
            %%ADD LFPS STILL INCOMPLETE 06/25
            %2020?
...
%make new directory for saving files for each align event
savepath=fullfile(pathfscv,'matlab',alignname,filesep);
if ~isdir(savepath)
    status = mkdir(savepath);
end
...
for ii=1:totaltrials
    ...
    if fscvTargetTrialsIDs(ii)<=length(fscvNlxEvents)
        ...
        if splitfiles==0
            ...
            save([savepath, saveName],'fscv','samplesLFP','tsLFP','NlxEventTTL','NlxEventTS','nlxFileNames','-v7.3');
        ...
        if splitfiles==1
            ...
            for nlxch=1:numLFPch
                ...
                save([savepath savecsc],'dg_Nlx2Mat_Samples','dg_Nlx2Mat_SamplesUnits','dg_Nlx2Mat_Timestamps','-v7.3');
            end
            %save fscv file without nlx signals, but with events
            save([savepath, saveName],'fscv','tsLFP','NlxEventTTL','NlxEventTS','nlxFileNames','-v7.3');
