function syncda(subject,sessnum,align,varargin)
%just da signals
log=[];
subjectname='patra';
...
%iscaps=strfind(filesfscv,'100_cvtotxt')
...
if isempty(firstFileIDs)
    ...
    ispresent=contains(filesfscv,'200_cvtotxt','IgnoreCase',true);
...
if ~strcmp(alignname,'fixedintervals')
                   ...
                   if rforward>rbackward
                       ...
                       fscvTargetTrialsIDs(trialidremove(1))=[];
                        ...
                        trialidremove=find(abs(discrepancy)>=medianlag);   
                            ...
                            [r,p]=corr(intervalstsFSCV,intervalstsNlx');
                            if r<0.95
                                error('tried to remove 2 trials, but not fixed, manually fix');
                            end
                         end
                   else
    ...
    %FIXED INTERVALS< DOES NOT WORK DO NOT USE
    %{
    preOffset=0;
    durationTrial=0;
    if mergeflag==1
...
for ii=1:totaltrials
    ...
    if fscvTargetTrialsIDs(ii)==previousID || lasttrialfinished
        continue
    end
   ...
   % currentLFPIndex=lfp_tID(ii)-nlxSamplesOffset;
    ...
    if ii<length(fscvTargetTrialsIDs) && nextFSCVIndex<=size(fscvNlxEvents,1) %&& nextLFPIndex<length(TS) 
     ...
     %   tsLFP=TS(currentLFPIndex:nextLFPIndex);
        nlxTargetIDsWithinTrial=find(nlx_ts>=fscvNlxTS(currentFSCVIndex) & nlx_ts<=fscvNlxTS(nextFSCVIndex));
        ...
        if fscvTargetTrialsIDs(ii)<=length(fscvNlxEvents)
            ...
            nlxTargetIDsWithinTrial=find(nlx_ts>=fscvNlxTS(currentFSCVIndex) & nlx_ts<=max(fscvNlxTS));
            NlxEventTTL=dg_Nlx2Mat_TTL(nlxTargetIDsWithinTrial);
            NlxEventTS=nlx_ts(nlxTargetIDsWithinTrial);           
        end
        lasttrialfinished=1;        %flag
    end
    if fscvTargetTrialsIDs(ii)<=length(fscvNlxEvents)
        ...
        if splitfiles==0
            ...
            save([PathName4, saveName],'fscv','NlxEventTTL','NlxEventTS','-v7.3');
