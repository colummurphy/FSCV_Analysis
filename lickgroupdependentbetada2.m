...
btrials{1}=(xbinfos(big).daall.trials);
btrials{2}=(xbinfos(sma).daall.trials);

bdata{1}=nanmean(xbinfos(big).daall.lfptracesaln(:,mididx:mididx+offset),2);
bdata{2}=nanmean(xbinfos(sma).daall.lfptracesaln(:,mididx:mididx+offset),2);
...
savename=[savepath 'sess' num2str(sessnum) '_win_' num2str(round(offset/rate)) 's'];
...
savename=[savepath 'sess' num2str(sessnum) '_' event '_' dapair{ip} '_' lfppair{ip} '_win_' num2str(round(offset/rate)) 's'];
