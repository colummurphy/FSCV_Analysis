function samples = deglitchandsmooth(samples, padpts,varargin)
...
%dgibson $
argnum=1;
samplerate=30e3;        %default sample rate patra/cleo cheetah non-down
while argnum <= length(varargin)
    switch varargin{argnum}
        case 'samplerate'
            argnum=argnum+1;
            %sample rate to calculate fillwidth for fillgap function
            samplerate=varargin{argnum};

    end
    argnum = argnum + 1;
end

fillwidth=samplerate/1000;
if fillwidth<=2
    fillwidth=3;
end
...
abssamp=abs(samples);
thresh=mean(difsamp)*7;     %arbitrarily set threshold for given signal based on mean differential
detectedDiffTrigs=find(difsamp>thresh); 
allTrigs=detectedDiffTrigs;     %merge timestamps found from both thresholds
...
for idx=1:length(uniqueArtifacts)
    if idx==1
        ...
        winNeg=idNeg-padpts;
        winPos=idPos+padpts;
        windowsmooth=winNeg:winPos;
        windowsmooth=windowsmooth(windowsmooth>0 & windowsmooth<length(abssamp));
        
        samples(windowsmooth)=nan;  
    end
end
  samples=fillgaps(samples,fillwidth);     %interpolate around nanned harmonic peaks in a smooth manner 05/05/2018
