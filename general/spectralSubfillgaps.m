...
%overlap=30;             % percent overlap of signal in time, increased form 5 to 10 07/01/2018
...
for chunkID=1:length(chunks)
   ...
   % smoothfft=smoothts(denoiseShiftMag,'g',avgSpectralWidth);       
    ...
    for ii=1:length(harmonicIDs)
        ...
        lowerBound=find(smoothfft(freq0ID:harmonicIDs(ii))<.25*max(smoothfft(harmonicIDs(ii)-avgSpectralWidth:harmonicIDs(ii)+avgSpectralWidth))); %determine width of noisy peaks, find border
        upperBound=find(smoothfft(harmonicIDs(ii):end)<.25*max(smoothfft(harmonicIDs(ii)-avgSpectralWidth:harmonicIDs(ii)+avgSpectralWidth)));
