function varargout = plotshaded(axa,x,y,fstr,alphashade)
% x: x coordinates
...
if size(y,1)==2 %plot shaded area
    ...
    py=[y(1,:), fliplr(y(2,:))];
    ap=patch(axa,px,py,1,'FaceColor',fstr,'EdgeColor','none');
end
 %{
if size(y,1)==3 % also draw mean
 ...
 %}
%ap.FaceAlpha(.2); % make patch transparent
set(ap,'FaceAlpha',alphashade);
end