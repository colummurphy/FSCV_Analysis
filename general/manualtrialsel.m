...
for ch=1:4 %added these loops but i dont think this will work unless we remove/change lines 28, and 29 since that sets goodtrials=alltrials
    if sum(~isnan(final(:,ch)))==0 %finds entirely empty channels
        chbadtrials(:,ch)=alltrials; %assigns them as bad trials
        goodtrials{ch}=[];
    else 
        goodtrials{ch}=alltrials(~ismember(alltrials,final(:,ch))); %for the empty trials in channels with noted bad trials, filters out the good trials 
    end
end
