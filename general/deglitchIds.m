function samples = deglitchIds(samples, ids, padpts,varargin)
%deglitch samples based on provided sample id points
...
allTrigs=ids;     %provides timestamps
...
for idx=1:length(uniqueArtifacts)
    if idx==1
        ...
        winNeg=idNeg-padpts;
        winPos=idPos+padpts;
        if winPos<=(length(samples)) && winNeg >=1
