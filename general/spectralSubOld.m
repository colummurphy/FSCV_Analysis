...
overlap=5;             % percent overlap of signal in time each side
chunkwidth=.5e6;             %num samples per chunk
chunkwidth=.1e6;             %num samples per chunk
...
for chunkID=1:length(chunks)
    ...
    for ii=1:length(harmonicIDs)
        ...
        %lowerBoundP=find(abs(denoiseShiftPhase(freq0ID:harmonicIDs(ii)))<.05*abs(denoiseShiftPhase(harmonicIDs(ii)))); %determine width of noisy peaks, find border
       % upperBoundP=find(abs(denoiseShiftPhase(harmonicIDs(ii):end))<.05*abs(denoiseShiftPhase(harmonicIDs(ii))));
        %if do not found bounds skip.
        if isempty(lowerBound) || isempty(upperBound)
            continue
        end
        harmonicWidthDown=harmonicIDs(ii)-freq0ID-lowerBound(end)+2;
        harmonicWidthUp=upperBound(1)+2;        
