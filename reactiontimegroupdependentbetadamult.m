...
for itype=1:2
    ...
    if ~binavg
    scatter(axa{itype},datop{itype},betatop{itype},10,'x','MarkerEdgeAlpha',1,'MarkerFaceAlpha',.5,'MarkerFaceColor',ctop,'markeredgecolor',ctop); 
    scatter(axa{itype},dabot{itype},betabot{itype},50,'o','MarkerEdgeAlpha',1,'MarkerFaceAlpha',.5,'MarkerFaceColor',cbot,'markeredgecolor',cbot); 
    else
        ...
        for ib=1:length(binedges)-1
         ...
         scatter(axa{itype},binedges(1:end-1)+.05,binavgbetatop{itype},50,'x','MarkerEdgeAlpha',1,'MarkerFaceAlpha',.5,'MarkerFaceColor',ctop,'markeredgecolor',ctop); 
         scatter(axa{itype},binedges(1:end-1)+.05,binavgbetabot{itype},50,'o','MarkerEdgeAlpha',1,'MarkerFaceAlpha',.5,'MarkerFaceColor',cbot,'markeredgecolor',cbot); 
         errorbar(axa{itype},binedges(1:end-1)+.05,binavgbetabot{itype},bincibetabot{itype}./1.96,'marker','none','linestyle','none','linewidth',1,'capsize',1,'color',cbot);
                  errorbar(axa{itype},binedges(1:end-1)+.05,binavgbetatop{itype},bincibetatop{itype}./1.96,'marker','none','linestyle','none','linewidth',1,'capsize',1,'color',ctop);
                   cla(axa{itype+2});
                  scatter(axa{itype+2},binedges(1:end-1)+.05,binavgdatop{itype},50,'x','MarkerEdgeAlpha',1,'MarkerFaceAlpha',.5,'MarkerFaceColor',ctop,'markeredgecolor',ctop); 
         scatter(axa{itype+2},binedges(1:end-1)+.05,binavgdabot{itype},50,'o','MarkerEdgeAlpha',1,'MarkerFaceAlpha',.5,'MarkerFaceColor',cbot,'markeredgecolor',cbot); 
...
if binavg
    savepath=[savepath 'binavg_'];
end
if discretecutoff
    savepath=[savepath 'cutoff_' num2str(cutoff{1}) 'top_' num2str(cutoffbot{1}) 'bot_' num2str(cutoff{2}) 'smtop_' num2str(cutoffbot{2}) 'smbot_'];
end
if grptypes
    savepath=[savepath 'grptypes_'];
end
if norm
    savepath=[savepath 'norm_'];
end
if rescale
    savepath=[savepath 'rescale_'];
end
if length(sessnums)>1
    savepath=[savepath 'multsess_'];    
else
    savepath=[savepath 'sess' num2str(sessnum) '_'];
end    
savename=[savepath  beh '_' event  '_win_' num2str(round(offset/rate)) 's_' condlabel];
