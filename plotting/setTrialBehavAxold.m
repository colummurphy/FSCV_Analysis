...
yyaxis right;
p2=plot(hfig{plotnum+7},1,.5:1:numtrials+.5,'marker','none');
ylim(hfig{plotnum+7},[0.5 numtrials+.5]);
yticks2=1:numtrials;
yticks2label=seltrials;

%yticks2label=fliplr(seltrials);
%{
if numtrials>50
    yticks2=(rem(numtrials,10):10:numtrials);
    yticks2(yticks2==0)=1;
    yticks2label=fliplr(seltrials(yticks2));
end
%}
set(hfig{plotnum+7},'ytick',yticks2,'ycolor',[0 0 0]); 
set(hfig{plotnum+7},'tickdir','out');
set(hfig{plotnum+7},'yticklabel',yticks2label)
set(hfig{plotnum+7},'fontsize',7)
hold(hfig{plotnum+7},'off')
set(hfig{plotnum+7},'YDir','reverse')        %flip y-axis values so first trial on top
