...
while argnum<=length(varargin)
    switch varargin{argnum}
        ...
        case 'beh'
            isbeh=1;
        case 'onlylfp'
            onlylfp=1;          %make up da as 'cp' channel so one fscv fake channel with all lfps
            noplot=1;
    end
    argnum=argnum+1;
end
...
maxplots=5;
if ~isbeh
    if ~isempty(data.lfp) && ~isempty(data.da)
    ...
    elseif isempty(data.da) && onlylfp
        pgroup=find(contains(lfpchs,'p')==1);
        cgroup=find(contains(lfpchs,'c')==1);
        maxplots=[length(pgroup)+length(cgroup)];
        fcvlfpgroups{1}.lsites=[pgroup cgroup];
        fcvlfpgroups{1}.fsite=1;
    end
else
...
if ~contains(evname,'outcome')
    ...
    if ii==1
        ...
        xcovdata{ii}=xvardata(xdata,xrates,plotparam,[],seltrials,...
        'sitename',sitenames,'fbands',plotparam.fbands,'type',vartype,'eventnames',evname, 'noplot','xwin',xwin,'single');    
    end
...
%end ii lfp types
    ...
    %just da
     xdata={[] da};
    sitenames={'' plotparam.sites{ida}};
    plotparam.alnts=alnts;
    xrates=data.rates;
    %get xvar's of data provided in xdata
    xcovdata=xvardata(xdata,xrates,plotparam,[],seltrials,...
        'sitename',sitenames,'fbands',plotparam.fbands,'type',vartype,'eventnames',evname, 'excludethres',.5,'noplot','xwin',xwin,'single');    
    if isempty(xcovdata)
        %did not process because too few trials, skip
        continue
    end    
    das=xcovdata.dadata;            %all trials shifted data, downconverted
    winids=xcovdata.winids;     %only seltrials
    seltrials=xcovdata.seltrials;      
    %get std of signal baseline in first half of each signal win
    curda=das(seltrials,winids);
    basedastds=nanstd(curda(:,1:round(median(1:size(curda,2)))),[],2);
    stdall=nanmean(basedastds); 
    xcovdata.daavgstds=stdall;%da baseline 
