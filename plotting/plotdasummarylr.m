function plotdasummarylr(datm,plotparam,varargin)
%left vs right instead of big vs small
...
condlabels={'L','R'};
plotnums=1:length(daregions)*length(sessiontypes);
plotvar={};
event='targ';
...
for ic=1:length(sessiontypes)
    if ~grpreg
                %separate plots  for different conditions & regions
    for ir=1:length(daregions)
        plotvar{ir+length(sessiontypes)*(ic-1)}=[dalabels{ir} '_' sessiontypes{ic}(1)];
    end
    else
         %separate plots for different conditions
        plotvar{ic}=[sessiontypes{ic}(1)];
    end
end
...
for ip=plotnums
    ...
    axpos{ip}=get(axa{ip},'position');
     if ~scattypes
    set(axa{ip},'xtick',1:length(conditions),'xticklabel',conditions);
    set(axa{ip},'xlim',[0 length(conditions)+1]);
     end
...
for is=1:length(sessnums)
    ...
    avgda={};
for icond=1:length(sessiontypes)
    nc=0;
    np=0;
    nn=0;
...
for ida=1:length(curda)
    ...
    danormval=1;
        dapertype={};
    for itype=1:length(conditions)
        %get median value for both types big/small
        targt=find(contains(trialinfo(icond).names,conditions(itype))==1);
        trialnums=trialinfo(icond).nums{targt};
        targsi=find([sites.sessnum]==sessnums(ise) & ...
            strcmp({sites.probeid},curda(ida)));
        if ~isempty(targsi)
        ich=sites(targsi(1)).ch;
        datarg=datm{ise}{icond}{ich};
        if ~isempty(datarg)
...
scatter(axa{pid},(1:length(conditions))+ranj(ise),...
...
if scattypes && length(conditions)==2
...
if ~meanda && ~scattypes
aline=plot(axa{pid},(1:length(conditions))+ranj(ise),avgy,'-','linewidth',1,'color',sitecolor);      
aline.Color(4)=.3;
end
...
for ip=plotnums
    ...
    aline=plot(axa{ip},(1:length(conditions)),meansda,'-','linewidth',1,'color',[0 0 0]);  
    aline.Color(4)=.7;
...
if scattypes
    ...
    for ip=plotnums
        ...
        aci.Color(4)=.25; 
        ylabel(axa{ip},['\Delta[DA] (nM) '  conditions{1}]);
    xlabel(axa{ip},['\Delta[DA] (nM) ' conditions{2}]);
    end
end
savename=[savepath event '_' targval '_lr'];
if normda
