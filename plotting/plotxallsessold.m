...
for did=1:length(curgrp)
    ...
    if itype==length(sessiontypes)       
        ...
        ystd=nanstd(getys);
        ymean=nanmean(abs(getys));
        if strcmp(curgrp{did},'zlagcoef')
            ymean=nanmean(getys);        %do not take absolute value
        end
...
save([savedir ynam{yid} '_' eventtypes{ievent} ...
    '_' targdasites{ida} 'x' lfpsites{ilfp}],'plotdata');
savefig(figsess,[savedir ynam{yid} '_' eventtypes{ievent} '_' targdasites{ida} 'x' lfpsites{ilfp}]);
saveas(figsess,[savedir ynam{yid} '_' eventtypes{ievent} '_' targdasites{ida} 'x' lfpsites{ilfp}],'tif')
