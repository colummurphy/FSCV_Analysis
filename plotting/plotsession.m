...
%NEED TO REMOVE smbshare from path after chunky upgrades 04/2019 from DG
if ~ispc
xx=strcmp(paths{1},'smbshare');
xz=findstr(paths{1},filesep);
paths{1}=paths{1}(xz(2):end);
xz=findstr(paths{2},filesep);
paths{2}=paths{2}(xz(2):end);
end
...
homedir='Z:';
if ~ispc
    homedir=[filesep];           %REMOVE SMBSHARE 04/2019
end
...
eventcodes={};
if strcmp(subjectname,'patra')
[param,csc_map,eventcodes]=getparams('patrabipolar','default',ncschannels,'sessnum',sessnum);
else
