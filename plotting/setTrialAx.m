...
iscleo=contains(plotparam.pathname,'cleo');
...
if iscsc && isempty(rclfp)
    ...
    idstargeye=round(plotparam.ttargeye.*samplespersec);
    if iscleo
        %05/15/2019, movement related artifacts remove trials
        %look around targ window for glitches, remove bad trials
        %figure; plot(da(seltrials(5),idstarg(seltrials(5))-1*samplespersec:idstarg(seltrials(5))+3*samplespersec))
        for ix=1:size(da,1)
            curwin=idstarg(ix)-1*samplespersec:idstarg(ix)+3*samplespersec;
            curwin=curwin(curwin>0 & curwin<=size(da,2));
            if isempty(curwin)
                badsigtrials=[badsigtrials ix];
            else
                stdsig=nanstd(da(ix,curwin));            
                if stdsig<3
                    %if std deviation < 3 microvolts, not physiological , maybe
                    %all interpolated during downconvert stage
                    badsigtrials=[badsigtrials ix];
                end
                difsig=diff(da(ix,curwin));    
                if any(abs(difsig)>60)
                    badsigtrials=[badsigtrials ix];
                end
            end
        end
        badsigids=find(ismember(seltrials,badsigtrials));
        seltrials(badsigids)=[];            %remove bad sig trials from seltrials
    end
    switch filtband
        case 'betal'
            bandfilt=filtbetal;            
...
if eyex==1 && isempty(rclfp)
   datatemp=[];
   meaneye=nanmean(da,2);
    meane=nanmean(meaneye);
   stdeye=nanstd(da,[],2);
    meanstd=nanmean(stdeye);
    threseye=abs(meane)+3*meanstd;
  for itrial=1:size(da,1)
      %REMOVE BLINKS
      datatemp=deglitchnanamp(da(itrial,:),threseye,30);
      %datatemp=da(itrial,:);
      da(itrial,:)=datatemp;
...
if pulse==1 && isempty(rclfp)
    %03/30/2019  instantaneous HR now defined by RR
    %interval rather than movin window
    sitename='pulse';
    %calculate HR
    pulsed=zscore(da,0,2);
    movingwindow=5;     %5 second long moving window
    stepsize=.1;       %0.5 second step size
    newrate=1/stepsize;
    ...
    samplespersec=newrate;       %DEFINE NEW SAMPLE RATE
    tslfp=tsdown;
    winneg=round(plotparam.alignidx-plotparam.win(1))/plotparam.samplespersec;
    winpos=round(plotparam.win(end)-plotparam.alignidx)/plotparam.samplespersec;
    rewardidx=round(plotparam.alignidx.*samplespersec./plotparam.samplespersec);
    ...
    idsfixeye=round(idsfixeye./ceil(data.ratelfp)*samplespersec);   
    idstargeye=round(idstargeye./ceil(data.ratelfp)*samplespersec);  
    da=hrdown;          %can back calculate rr interval using 60/hrdown
end
info.ts=tslfp;
...
postrewwinvpretarg=nan(1,numtrials);
postrewpeakvpretarg=nan(1,numtrials);
rewshortpeakvpretarg=nan(1,numtrials);
rewshortwinvpretarg=nan(1,numtrials);
rewimpeakvpretarg=nan(1,numtrials);
rewimwinvpretarg=nan(1,numtrials);  
...
if badtrls(tid)==0 && ~isnan(idsfix(tid))
    ...
    if ~any(targbaselineids<1) && ~strcmp(plotparam.trialtype,'fixbreak') &&...
       ...
       if ~any(isnan(posttargmax)) && nanratio<=.5
        ...
        %outcome charactersits with pretarg baselien
        reidx=idsrew(tid);
         if eyex
            %eye x signal need to take before cue appears
        ...
        postrewwinvpretarg(xx)=nanmean(da(tid,rewids))-targbaseline;
        rewdasub=da(tid,rewids)-targbaseline;
        ...
        if ~any(isnan(postrewmax)) && nanratio<=0.5
            ...
            postrewpeakvpretarg(xx)=da(tid,postrewpeakidx)-targbaseline; %relative max change
        end        
        
        %reward peak short window (3s)
        rewids=reidx+pade:reidx+postrewdurshort;
        rewshortwinvpretarg(xx)=nanmean(da(tid,rewids))-targbaseline;
        rewdasub=da(tid,rewids)-targbaseline;
        ...
        if ~isnan(postrewmax)
            ...
            rewshortpeakvpretarg(xx)=da(tid,maxidx)-targbaseline; %relative max change
        end
        
        %reward peak relative to pre baseline immediate , no pad for im    
        rewids=reidx:reidx+postrewdurim;
        rewimwinvpretarg(xx)=nanmean(da(tid,rewids))-targbaseline;
        rewdasub=da(tid,rewids)-targbaseline;
        [postrewmax,postrewmaxidxrel]=max(rewdasub);
        if ~isnan(postrewmax)
            ...
            rewimpeakvpretarg(xx)=da(tid,maxidx)-targbaseline; %relative max change
        end
...
plotdata.targpeakabsci=nanstd(plotdata.targpeakabs)/sqrt(length(find(~isnan(posttargpeakabs))))*1.96;  %95% confidence interval
plotdata.targpeakabsavg=nanmean(plotdata.targpeakabs);
...
plotdata.rewwinvtarg=postrewwinvpretarg;
plotdata.rewpeakvtarg=postrewpeakvpretarg;
            
plotdata.rewshortpeakvtarg=rewshortpeakvpretarg;
plotdata.rewshortwinvtarg=rewshortwinvpretarg;

plotdata.rewimpeakvtarg=rewimpeakvpretarg;
plotdata.rewimwinvtarg=rewimwinvpretarg;
