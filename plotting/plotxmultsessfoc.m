function plotdata=plotxmultsessfoc(xinfos,plotparam,trialtype,yvar,varargin)
...
%1/21/2019, modified from plotxmultsessfoc to plot just specified
%trialtypes 'all', 'sleft', etc. 
%and yvar.ygrp= & yvar.ynam (names of groups for labeling)
argnum=1;
datype='dapos';
rate=10;
...
savepath=[savepath(1:end-1) 'foc' filesep];
if ~isdir(savepath)
    mkdir(savepath);
end
...
params_timing_damax={'delt_lfpmax_damax','delt_lfpfall_damax'...
    ...
    'delt_lfppostmax_damax','lfppostmaxts','damaxts','lfpmaxts'}; 
...
for yid=1:length(ygrp)    
    ...
    axpos={};
    for ip=1:8
        axa{ip}=subplot(2,4,ip);   hold(axa{ip},'on');
        set(axa{ip},'units','pixels');
        axpos{ip}=get(axa{ip},'position');
            %left plot            
            set(axa{ip},'position',[axsiz(1)*(rem(ip-1,4))+75*(rem(ip-1,4))+100 axpos{ip}(2) axsiz(1) axsiz(2)]);
    ...
    %set legend
    labelx={};
        text(axa{1},220,axpos(4)+30,'session #','units','pixels','fontweight','bold');
    for xleg=1:length(xinfos)
        labelx{xleg}=[num2str(sessnums(xleg))];  
        axpos=get(axa{1},'position');
        text(axa{1},280+xleg*50,axpos(4)+30,labelx{xleg},'units','pixels','fontweight','bold','color',mark(xleg,:));
        text(axa{1},280+xleg*50+18,axpos(4)+30,sessmarktxt{xleg},'interpreter','tex','units','pixels','fontweight','bold','color',[0 0 0]);
...
for itype=1:length(sessiontypes)
    ...
    itrial=1;
    trialnums=trialtypes.nums{strcmp(trialtypes.names,trialtype)};
        grptids(itrial).name=trialtype{:};
        grptids(itrial).nums=...
            find(ismember(curdata.trials,trialnums)==1);
        ...
        if ~strcmp(trialtypes.names{itrial},'switch')
            ...
            allbutsw=curdata.trials(ismember(curdata.trials,trialnums)...
...
for did=1:length(curgrp)
    ...
    for ix=1:length(xdata)
        ...
        if ~isempty(grpdata(gids{ix}))
            ...
            if contains(curgrp{did},'delt')
                %timing, multiply by rate
                ydata{ix}=ydata{ix}./rate;
            end
            ...
            scatter(axa{pid},itype+xoff,...
                avgy,65,sessmark{xx},'markeredgecolor',cmark,...
    ...
    if itype==length(sessiontypes) && xx==length(sessnums)       
        ...
        if ~isnan(ystd) && ~isempty(ystd)
            set(axa{pid},'ylim',[ymean-ystd*.5 ymean+ystd*.5]);  
        end
    end
    cntp=cntp+1;
    set(findall(axa{pid},'-property','FontSize'),'FontSize',fontsize)
    set(axa{pid},'xlim',[0.15 length(sessiontypes)+0.75])
end
end
end
end
...
for isess=1:length(sessnums)
  ...
  curids=find(contains({plotdata.yvar},yvars{iy})==1 ...
    ...
    sesstypescur={plotdata(curids).sesstype};
    xtypes=find(ismember(sessiontypes,sesstypescur));
    ploty=[plotdata(curids).yavg];
    lp=plot(axa{pid},xtypes+xoff,ploty,'-','linewidth',.5,'color',cmark);
    lp.Color(4)=0.3;
end
%end
    cntp=cntp+1;
