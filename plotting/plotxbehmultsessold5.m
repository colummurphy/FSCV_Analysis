...
for iitype=1:length(targrow)
    xdata=getfield(xinfos{ises}(targrow(iitype)),datype);
    ...
    curxtrials=[];
    tidx=0;
    targb=find(contains({binfos{ises}.siteda},targdasites(ida)) & ...
    (contains({binfos{ises}.sessiontype},sessiontypes(itype)) &...
    strcmp({binfos{ises}.event},eventtypes(ievent)))==1 & ...
    strcmp({binfos{ises}.sitelfp},bparams{ib})==1);
    for iitype=1:length(targb)
        curbdata=binfos{ises}(targb(iitype));
