...
while argnum<=length(varargin)
    switch varargin{argnum}
       ...
       case 'uiponly'
        ...
        case 'mintrials'
            %for history effect accumulated more than 1 trial
            argnum=argnum+1;
            mintrials=3;
            mintrials=varargin{argnum};
...
if xflag
    titletext=[event ' | ' targval ' | win ' num2str(xwin(1)+offset(1)) ' - ' num2str(xwin(2)+offset(2)) ' s' ];  
end
...
if ~isempty(datarg)
    ...
    if xflag
        ...
        trialids=find(ismember(datarg.trials,trialnums)==1); %trial ids for targeted metric
         ...
         trialnums=intersect(datarg.trialnums,trialnums);
    ...
    tnums{pid}{sitenum}{icond}=[tnums{pid}{sitenum}{icond} trialnums];  
end
end
%finish getting group values
sitevals(countnum).vals{icond}=yvals{pid}{sitenum}{icond};
sitevals(countnum).trialnums{icond}=tnums{pid}{sitenum}{icond};
    ...
    for itt=1:2:length(ttypes)
        ...
        if isnan(sig((itt+1)/2))
         ...
         %store significance and non-sig results for each combo of conditions tested
            countsig{(itt+1)/2}(sitenum).ttype=[[ttypes{itt}{:} '.'] [ttypes{itt+1}{:}]];      %first condition vs 2nd cond group
            countsig{(itt+1)/2}(sitenum).dasite=sitevals(sitenum).site{:};
            countsig{(itt+1)/2}(sitenum).sessnum=sitevals(sitenum).sessnum;
            countsig{(itt+1)/2}(sitenum).sig=sig((itt+1)/2);
            countsig{(itt+1)/2}(sitenum).p=pval;
            countsig{(itt+1)/2}(sitenum).pos=nanmean(dapercond{itt})<nanmean(dapercond{itt+1});     %positive slope from cond 1 to cond 2
           % countsig{itt}(sitenum).plotvals=sitevals(sitenum).vals;
            countsig{(itt+1)/2}(sitenum).day=sitevals(sitenum).day;
            countsig{(itt+1)/2}(sitenum).sensortype=sitevals(sitenum).type;
    end
    %sig for pairs
end
...
if scattypes && length(ttypes)==2 && ~plotsens
...
for ip=plotnums
    ...
    for iip=1:length(cilines)
        ...
        if mod(iip,2)
            ...
            totalsig=length(find([countsig{(iip+1)/2}.sig]>0));
            siteids=find(contains({countsig{(iip+1)/2}.dasite},cnl(ip)));
            totalsites=length(siteids);
            %ax=text(axa{iip},iip,ylims(2),[num2str(totalsig) '/' num2str(totalsites)],'fontweight','bold');
            totalsigpos=length(find([countsig{(iip+1)/2}(siteids).sig]>0 & [countsig{(iip+1)/2}(siteids).pos]==0));
            totalsigneg=length(find([countsig{(iip+1)/2}(siteids).sig]>0 & [countsig{(iip+1)/2}(siteids).pos]==1));
            ax=text(axa{ip},iip,ylims(2),['+' num2str(totalsigpos) '/' num2str(totalsites) sprintf('\n') '-' num2str(totalsigneg) '/' num2str(totalsites)]);
 ...
 %}
    ...
    aci=plot(axa{1},[ip ip],[meansdapos(ip)-cisdapos(ip) meansdapos(ip)+cisdapos(ip)],'-','linewidth',3,'color',[0 0 0]);    
        ...
        %label number sig sites / total for each tested pair
            ...
            totalsig=length(find([countsig{1}.sig]>0));
            totalsites=length(countsig{1});
            %ax=text(axa{iip},iip,ylims(2),[num2str(totalsig) '/' num2str(totalsites)],'fontweight','bold');
            csites=find(contains({countsig{1}.dasite},'c'));
            psites=find(contains({countsig{1}.dasite},'p'));
            siteids=csites;
            if ip==2
                siteids=psites;
            end
            totalsigpos=length(find([countsig{1}(siteids).sig]>0 & [countsig{1}(siteids).pos]==0));
            totalsigneg=length(find([countsig{1}(siteids).sig]>0 & [countsig{1}(siteids).pos]==1));    
            ax=text(axa{1},ip,ylims(2),['+' num2str(totalsigpos) '/' num2str(length(siteids)) sprintf('\n') '-' num2str(totalsigneg) '/' num2str(length(siteids))]);      
...
if xflag
    savename=[savename '_xdat'];
     savename=[savename '_win_' num2str((xwin(1)+offset(1))*ratefscv) '-' num2str((xwin(2)+offset(2))*ratefscv) ];
end
if plotv
...
save(savename,'groupvals','countsig');
