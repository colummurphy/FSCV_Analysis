...
plotparam.trialtypes=trialtypes;    %needed for xplot
%{
for ilfp=1:maxplots 
for ievent=1:length(targevents)
    figsess{ilfp,ievent}=figure('position',[50,50,1800,1000],'color',[1 1 1]);
    set(0,'CurrentFigure',figsess{ilfp,ievent});    %set figure handle to current figure
    iplot=1;
    for ii=1:length(fscvchs)
        axplot{ilfp,ievent}.axa{ii,1}=subplot(2,length(fscvchs)*2,iplot);
        iplot=iplot+1;
        axplot{ilfp,ievent}.axb{ii,1}=subplot(2,length(fscvchs)*2,iplot);
        iplot=iplot+1;
        axplot{ilfp,ievent}.axa{ii,2}=subplot(2,length(fscvchs)*2,iplot);
        iplot=iplot+1;
        axplot{ilfp,ievent}.axb{ii,2}=subplot(2,length(fscvchs)*2,iplot);
        iplot=iplot+1;
    end
end
end
...
typecount=typecount+1;
    ...
    plotparam.chnums=fscvchs;    
    for it=1:length(trialtypes.names)
        %scroll trial types, ie. phase 1 phase 4. etc.
        trialname=trialtypes.names{it};
        if selbands(ifreq)==2
            trialname=[trialname '2'];
        end
    ...
    save([savepath 'xinfo' ], 'xinfo');
plotparam.savepath=savepath;        %needed for xplot
plotx(xinfo,plotparam);
end
    


%ONLY USE INTERTARG AND INTERFIX EVENTS for burst groups
targevents={'interfix','intertarg'};
     
%plot burst groups vs no burst groups
siglabels={'burstgrp','noburstgrp','dapos','daneg'};
plotparam.trialtypes.names=siglabels;
fscvchstemp=fscvchs;        %will change when open compiledname
...
currpath=getfield(targpath,trialgroups{itype});
disp(currpath)
%get behavioral properties of burst groups
compiledname=[currpath 'trials_chronic' sessid '_' trialgroups{itype} '_pro'];
load(compiledname);
fscvchs=fscvchstemp;
...
plotparam.chnums=fscvchs;   
    ...
    for it=1:2
        %scroll burst vs no burst
        trialname=siglabels{it};
        typename='_all';
        plotparam.triallabel=trialname;
        if selbands(ifreq)==2
            typename=[typename '2'];
        ...
        for ii=1:length(fscvchs)
            ...
            for ievent=1:length(targevents)
                xinfotemp=[];
                %scroll event window types
                pathxvar=[currpath 'xvar_' targevents{ievent} '_' ...
                    vartype typename '_' trialname filesep];
                load([pathxvar 'xvar_' sites{ich}],'xcovdata');
                if isempty(xcovdata)
                    continue
                end
                xids=find(~cellfun(@isempty,xcovdata)); % get only xcvodata non-empty
                if isempty(xids)
                    continue
                end
                xcovdata2=xcovdata(xids);
                xcovdata=xcovdata2;
                if isfield(xcovdata{1},'sitenameda')
                if ~strcmp(xcovdata{1}.sitenameda,sites{ich})
                    %not stored, data just stored leftover from prev
                    %analysis, skip
                    warning(['improper stored xvar: ' pathxvar 'xvar_' sites{ich}]);
                    continue
                end
                else
                if ~strcmp(xcovdata{1}.sitename{2},sites{ich})
                    %not stored, data just stored leftover from prev
                    %analysis, skip
                    warning(['improper stored xvar: ' pathxvar 'xvar_' sites{ich}]);
                    continue
                ...
                xinfotemp=xclust(xcovdata,plotparam,'alllags','behav',trialbytrial(1),'event',targevents{ievent});                
                xinfo=[xinfo xinfotemp];        %concatenate all info
...
save([savepath 'xinfo_burstgrps'], 'xinfo');
plotparam.savepath=savepath;        %needed for xplot
plotx(xinfo,plotparam,'label','burstgrps_');
plotxb(xinfo,plotparam,'label','burstgrps_');
end
