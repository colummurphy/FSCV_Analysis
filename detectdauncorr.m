...
numpcs=3;       %da,ph,bg (NOW DA, PH, M) (Previously only used 2 pcs)
...
winb=120;   
winb=45;        %in seconds changed 09/28/2018, 2 min for bg sub point, 2/9/2019 changed from 60 to 45
...
winp=10;            %10 s too long 2/9/2019, changed to 7 s
winp=7;
winpstep=5;       %step size for sliding window for local peak from pcda
winpstep=.7;       %09/28/2018step size for sliding window for local peak from pcda

percentout=0.3;     %if >60% of window is bad then skip to next bg win sub
maxdurpeak=parameters.maxDurationPeak;
maxreturnlength=parameters.maxDurationPeak; %how long it takes for peak to decrease in samples
...
flagabsthres=0;
if isfield(parameters,'absthres')
if parameters.absthres
    %use absolute threshold for peak detection, not std deviation below
    flagabsthres=1;
end
end
...
badoverlap=[10 25];      %before/after these samples of movement artifact do not search
...
for iwin=1:winbs-1:fullsize(2)-10
   ...
   % windur=abs(tstart-tend);    %time period for finding correlating signals from BG point
    ...
    count=1;
    candidateids=[];
    for tpoint=5:10:length(idswinbg)-winbovs
    ...
    %if background subtraction at ideal point still creates data with a lot
    %of unwanted components, skip
    rr=corr(subdata,KDA);     %detect positive corr's to DA, not abs
    rda=find(rr>Rthres);        %idx's of correlated DA in window
    rm=corr(subdata,KM);     %detect positive corr's to m, not abs
    rmout=find(abs(rm)>parameters.RThresOut);
    ...
    daproj=ipcr.DAiso;
    Q=ipcr.Qplot;
    badzone2=find(Q>Qthres);
    badzone2=[badzone2 find(ipcr.Mplot>nanstd(ipcr.Mplot)*3)];
    badzone2=sort(unique(badzone2));
    badzone2(ismember(badzone2,glitchidsrel))=[];
    ...
    for iiwin=1:winpsteps:length(idswinbg)
        ...
        if size(parameters.K,2)<4
                    dasub=nanmean(daprojwin(nonnanids(1:10)));       %subtracted to beginning of window
        end
       ...
       % dasubend=nanmean(daprojwin(nonnanids(end-4:end))); 
        ...
        if mindiff>parameters.peakDiff || flagabsthres
        ...
        %added 2/9/2019, badzone 2 is Q > 10
        if any(ismember(maxts-badoverlap(2):maxts+badoverlap(1),...
                badzone2))
            continue
        end        
        %11/06/2018
        ...
        [maxslope,maxdiffpre]=max(diffda(1:localmaxfirst));  %max incline
        ...
        peakdeclineids=find(diffda(maxdiffpre:end)<=0)+maxdiffpre-1;    %declines when slope <=0
        if isempty(peakdeclineids)
            %signal does not plateau or decrease
            continue
        end
        firstdecline=find(diff(peakdeclineids)>1);
        limdeclineidx=length(diffda);      %default
        if ~isempty(firstdecline)
            firstdecline=firstdecline(1);
            limdeclineidx=peakdeclineids(firstdecline);
        end  
        %search local win between max rise & fall
        localwin=maxdiffpre:limdeclineidx;
        %get peak da signal between these 2 slopes 
        [peakdaredo,firstpeaktempid]=max(dasubwinlong(localwin));
        firstpeakredoid=firstpeaktempid+localwin(1)-1;
        if isempty(firstpeakredoid)
                %occurs only when values are nan in localwin
                continue
        end
        localmaxid=firstpeakredoid(1);
        ...
        %2/9/2019 ADD FIND DA CORRELATION
        %find correlated DA Cv's near or at peak
        %sub data by optimal idx
        %bg=data(:,optidx-BGavg:optidx+BGavg);
        %bgmean=mean(bg,2);
        %refmatrix=repmat(bgmean,1,size(data,2));   
        ...
        rdaid=[];
        distancetor=inf;
        dacv=nan(size(subdata,1),1);
        dacvts=nan;                 %ts (in sample id like maxts) for dacv
        if ~isempty(rda)
            [~,closestrda]=min(abs(rda-maxts)); %find closest correlated signal
            rdaid=rda(closestrda);
            distancetor=abs(rdaid-maxts);
            dacv=subdata(:,rdaid);
            dacvts=rdaid;
        end
        if distancetor>10 || isempty(rda)
            %greater than 1 second away, look for closer signal
            for bgid=optidx-50:optidx+50
                if bgid-BGavg>0 && bgid+BGavg<size(data,2)
                    bgnew=data(:,bgid-BGavg:bgid+BGavg);
                    bgmeannew=mean(bgnew,2);
                    refmatnew=repmat(bgmeannew,1,size(data,2));
                    subdatanew=data-refmatnew;
                    rr=corr(subdatanew,KDA);
                    rda=find(rr>Rthres);
                    if ~isempty(rda)
                        [~,closestrda]=min(abs(rda-maxts)); %find closest correlated signal
                        rdaid=rda(closestrda);
                        distancetor=abs(rdaid-maxts);
                        dacv=subdatanew(:,rdaid);
                        dacvts=rdaid;
                        if distancetor<=10
                            break
                        end
       ...
       % dasorted=sort(dasubwin,'ascend');
        ...
        %store acquired da corr cv's
        detected.cv(:,dacount)=dacv;
        detected.cvts(dacount)=dacvts+relid;
        detected.maxid(dacount)=maxts+relid;
