...
if isfield(fscvdata,'recordedData')
    ...
    tss=fscvData(:,1)-fscvData(1,1);
    tss1=round(tss*1e6)./1e6;       %round tss to nearest 1 microseconds
    res=find(tss1<(1/param.samplerate-.001));   
    samplesperscan=length(res);
    firstCH=3;          %1st col = timestamp, 2nd col = filt ramp in, 3rd col = ch 1 (ai1), 19th col = clk in
    firstCH=2;          %digital daq 1/18/2018 1st col = timestamp, ...
    %2nd col = ch 1 (ai1), 18th col = ai31=aisense (inverted ramp), 
    %19thcol=aisense, 20thcol=_ao0_vs_aognd(rampout)
    numch=numcols-firstCH;
    numch=numcols-firstCH-4-3+1;      %minus digital inputs & ramp monitors 01/13/18
    Irange=[];      %generate Irange structures containing anodal/cathodal currents and indeces for max/min voltages
    t_Ihalf=0;
    %first column in loaded data represents time stamps, 2nd column = triangle
    %wave, 3rd column = custom clock for synchronizeing
    if isempty(param.resort)
        param.resort=(1:32);
    end
    resort=param.resort;
    for ii=1:numch
        %reshape to map current across time (x-axis) w/ voltage distribution on y-axis
        reshapeScansPerFile=size(fscvData,1)/samplesperscan;
        if size(fscvData,1)/samplesperscan~=round(size(fscvData,1)/samplesperscan)
            error('size of recorded file not divisible by scans')
        end
        arrayedData=reshape(fscvData(:,resort(ii)+firstCH-1),samplesperscan,reshapeScansPerFile);
        ...
        Iread(ii).LPFdata=Iread(ii).data;
                Iread(ii).rawdata=Iread(ii).data;

        if any(ii==param.badChannels) %if a bad channel exists set this array to zero
            Iread(ii).data=ones(size(Iread(ii).data));
            Iread(ii).LPFdata=ones(size(Iread(ii).data));
            Iread(ii).rawdata=ones(size(Iread(ii).data));
        end
    end
    %Iread(7).data=Iread(9).data.*IVconv;
    if param.differentialChannel>0 %if differential ch exists
        for ii=1:length(Iread)
            if ii~=param.differentialChannel
