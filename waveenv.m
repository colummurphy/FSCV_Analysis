function enveloped = waveenv(data, varargin)
%copied from dan code lfp_waveenv 04/02/2018
...
%{
if isempty(mask)
    wave = reshape(lfp_Samples{filenum}(:), 1, []);
else
    zeros2addidx = find( ~mask(1:end-1) & mask(2:end) ...
        | ~mask(2:end) & mask(1:end-1) );
    wave = zeros(1, numel(lfp_Samples{filenum}));
    wave(mask) = lfp_Samples{filenum}(mask);
end
%}
...
if isempty(mask)
    enveloped = interp1(xvals, yvals, 1:length(wave), method);
else
    ...
    vals = sortrows(vals);
    enveloped = interp1(vals(:,1), vals(:,2), 1:length(wave), method);
end
end
%     xvals = zeros(1, length(peaks) + 2 + ...
%         sum(leadingzeros(:)) + sum(trailingzeros(:)));
