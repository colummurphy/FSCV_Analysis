function detected=detectdatransients(data,parameters,glitchids,tParam,varargin)
...
%use supplied time window for BG cursor, with parameters.BGwin param offsets
if ~isempty(tParam)
if ~isempty(tParam.timeWin)
    tstart=tParam.timeWin(1)*samplerate-parameters.BGWin(1);
    tend=tParam.timeWin(2)*samplerate+parameters.BGWin(2);
    if tstart<1
        tstart=5;
    end
    if tend>sizedata(2)
...
for tpoint=tstart:10:tend 
    ...
    ipcr = getpctdirect(subdata,'ats',ats,'cts',cts,...
        ...
        'imaxbg',max(abs(bgmean)),'glitchids',glitchids);
    daproj=ipcr.DAiso;
       ...
       % baselineda=mean(dasorted(baselineids),'omitnan');
        ...
        if any(ismember(maxts-overlap:maxts+overlap,...
            ...
            if length(storedidx)>1
                ...
                storedidx=storedidx(idclosestt);
            end
