function Ipcr = setguipct(axPlot, pct, plotParam,parameters,colorfscv,varargin)
...
if sum(pct.pHplot>0) && dispOthers(1)~=0
    ...
    plot(time_window,pct.pHplot(time_window),'color',colorfscv,'linestyle',':')
...
if sum(pct.Mplot>0) && dispOthers(3)~=0
    ...
    hold(axPlot,'on')
    set(axPlot,'ycolor',[ 0 0 0]) 
    plot(time_window,abs(pct.Mplot(time_window)),'color',colorfscv,'linestyle','--','linewidth',0.5)
    yyaxis('left');
end    
if sum(pct.BGplot>0) && dispOthers(2)~=0
    ...
    hold(axPlot,'on')
    set(axPlot,'ycolor',[ 0 0 0]) 
    plot(time_window,pct.BGplot(time_window),'color',colorfscv,'linestyle','-.','linewidth',0.5)
    yyaxis('left');
end
if sum(pct.Iox>0) && dispOthers(4)~=0
    ...
    hold(axPlot,'on')
    set(axPlot,'ycolor',[ 0 0 0]) 
    plot(time_window,pct.Iox(time_window),'color',colorfscv,'linestyle','-.','linewidth',0.5)
    yyaxis('left');
end

plot(time_window,pct.DAiso(time_window),'color',colorfscv,'linewidth',1,'marker','none')
if plotnum<=4
ylabel('[DA] (nM)');
end
set(axPlot,'ycolor',[ 0 0 0]); 
...
if scaleBar~=0 && scaleBarConc~=0
    ...
    %plot conc scale bar
    plot([time_window(5); time_window(5)], [scaleBarConc+5; 5], '-k',  'LineWidth', 1);
end
...
if ~isempty(events) && plotnum==1
    ...
    if ~isempty(eventsbre)
        for ii=1:size(eventsbre,2)
            x1=eventsbre(ii);
            plot([x1 x1],YLim,'color',[1 0 0],'linestyle','-','linewidth',...
                2,'marker','none');
        end
    end
    if ~isempty(eventsbig)
        for ii=1:size(eventsbig,2)
            x1=eventsbig(ii);
            plot([x1 x1],YLim,'color',[0 .7 .4],'linestyle','--','marker','none');
        end
    end
    if ~isempty(eventssmall)
        for ii=1:size(eventssmall,2)
            x1=eventssmall(ii);
            plot([x1 x1],YLim,'color',[0 0 .5],'linestyle','--','marker','none');
        end
    end
    ...
    %txt1=num2str(events2(ii,1));
        if events2(ii)~=0
            ...
            plot([x1 x1],YLim,'color',[.6 .6 .6],'linestyle','--','marker','none');
            end
        end
    end
...
if plotParam.plotbad && plotnum==1
    ...
    %text(badinwin(ii)-plotParam.t_start,YLim(2),'x','fontsize',10,'horizontalalignment',...
     %   'center','color',[1 0 0]);
    plot([badinwin(ii)-plotParam.t_start badinwin(ii)-plotParam.t_start],YLim,'color',[.6 .6 .6],'linestyle',':','marker','none');
    end
end
hold(axPlot,'off')
